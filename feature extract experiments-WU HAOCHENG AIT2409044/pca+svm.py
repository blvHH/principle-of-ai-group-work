...
# 5. PCA
'''
for i in [100,128,256,512,1024]:
    pca_components=i
    print("开始PCA")
    pca = PCA(n_components=pca_components)
    X_train_pca = pca.fit_transform(X_train_scaled)
    X_test_pca = pca.transform(X_test_scaled)
    joblib.dump(pca, os.path.join(output_dir, f"SVM_pca_{pca_components}.joblib"))
    # 6. SVM 训练
    print("开始训练")
    svm = SVC(kernel='rbf', probability=True,class_weight='balanced')
    svm.fit(X_train_pca, y_train)
    joblib.dump(svm, os.path.join(output_dir, f"svm_model_{pca_components}.joblib"))
    # 7. 模型评估
    ...
    report_df.to_csv(os.path.join(output_dir, f"SVM+PCA_classification_report{i}.csv"), index=True)
'''
pca_components=80
print("开始PCA")
pca = PCA(n_components=pca_components)
X_train_pca = pca.fit_transform(X_train_scaled)
X_test_pca = pca.transform(X_test_scaled)
joblib.dump(pca, os.path.join(output_dir, f"SVM_pca.joblib"))
# 6. SVM 训练
print("开始训练")
svm = LinearSVC(class_weight='balanced')
svm.fit(X_train_pca, y_train)
joblib.dump(svm, os.path.join(output_dir, f"svmlinear_model.joblib"))
# 7. 模型评估
y_pred = svm.predict(X_test_pca)
report = classification_report(y_test, y_pred, target_names=le.classes_)
print(report)
...
report_dict = classification_report(y_test, y_pred, target_names=le.classes_, output_dict=True)
report_df = pd.DataFrame(report_dict).transpose()
report_df.to_csv(os.path.join(output_dir, f"SVMlinear+PCA_classification_report.csv"), index=True)
