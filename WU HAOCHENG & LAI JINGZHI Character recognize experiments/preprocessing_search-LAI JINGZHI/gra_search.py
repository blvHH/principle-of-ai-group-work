...
def gradient_search_preprocess(img: np.ndarray,
                                label: str,
                                feature_func=apply_preprocessing,
                                score_func=score_image,
                                init_params: Dict = None,
                                lr: float = 0.1,
                                iters: int = 20,
                                eps: float = 1e-4) -> Tuple[Dict, np.ndarray, float]:
    """
    使用数值梯度下降搜索最佳预处理参数组合。

    参数：
        img: 原始图像（np.ndarray）
        label: 图像真实标签（用于 score_image 评估）
        feature_func: 图像预处理函数（默认 apply_preprocessing）
        score_func: 评分函数（默认 score_image）
        init_params: 初始参数字典，默认无
        lr: 学习率（步长）
        iters: 迭代轮数
        eps: 数值梯度用的扰动
    ...
    best_proc = feature_func(img, params)
    best_score = score_func(best_proc, label)
    ...
    for _ in range(iters):
        ...
        for k, v in params.items():
            ...
            temp_params[k] = v + eps
            proc_temp = feature_func(img, temp_params)
            score_temp = score_func(proc_temp, label)
            grads[k] = (score_temp - best_score) / eps
        ...
        # 应用新参数并评分
        proc = feature_func(img, params)
        score = score_func(proc, label)
...
def cluster_and_optimize(
    ...
    for cluster in range(n_clusters):
        ...
        for img, label in zip(selected_images, selected_labels):
            ...
            best_params, _, score = best_params, best_proc, best_score = gradient_search_preprocess(img, label)
    ...
    # 保存聚类参数和K-means模型
    with open('cluster_params_GRA.pkl', 'wb') as f:
