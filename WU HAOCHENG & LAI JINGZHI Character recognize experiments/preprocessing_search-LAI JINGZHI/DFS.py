...
def full_dfs_search(img, template,
                    steps=None,
                    time_limit=None,
                    score_tol=1e-3,
                    no_improve_limit=10):
    """
    基于 DFS 枚举所有预处理参数组合，并支持超时和无提升早停。
    img         : 输入灰度图
    template    : 目标模板图像
    steps       : 参数列表，每项{'param':str,'values':iterable}
    time_limit  : 最大搜索时间（秒），None 不限制
    score_tol   : 得分提升阈值，小于该值计为无改善
    no_improve_limit: 连续无改善次数阈值
    返回: best_params, best_proc, best_score
    """
    # 默认参数空间
    if steps is None:
        steps = [
            {'param': 'rot',   'values': np.arange(-15.0, 15.1, 3.0)},
            {'param': 'scale', 'values': np.arange(0.7,   1.31, 0.1)},
            {'param': 'alpha', 'values': np.arange(0.7,   1.31, 0.1)},
            {'param': 'sharp', 'values': np.arange(0.7,   1.31, 0.1)},
        ]
    param_keys = [step['param'] for step in steps]
    value_lists = [step['values'] for step in steps]
    ...
    # 递归 DFS
    def dfs(idx, current):
        nonlocal best_score, best_proc, best_params, no_improve
        # 超时检查
        if time_limit and (time.time() - start_time) > time_limit:
            return True
        # 到达叶子节点，评估
        if idx == len(steps):
            proc = apply_preprocessing(img, current)
            ...
            if score > best_score + score_tol:
                ...
                no_improve += 1
            # 无改善达到上限，提前停止全部搜索
            return no_improve >= no_improve_limit
        # 遍历当前参数所有可能值
        key = param_keys[idx]
        for v in value_lists[idx]:
            current[key] = float(v)
            stop = dfs(idx+1, current)
            if stop:
                return True
        # 回溯
        del current[key]
