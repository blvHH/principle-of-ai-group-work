...
knn_model = joblib.load(r"knnonly_model.joblib")
...
def cluster_and_optimize(
    ...
    for cluster in range(n_clusters):
        ...
        for img, label in zip(selected_images, selected_labels):
            ...
            best_params, _, score = full_astar_search(img, label)
...
if __name__ == "__main__":
    # 训练
    data_dir = r"Bmp"  # 数据集路径，包含多个类别文件夹
...
def full_astar_search(img, template,
                      init_params=None,
                      heuristic=None):
    import heapq
    ...
    if heuristic is None:
        heuristic = lambda p, s: s

    start_params = {k: np.mean(rng) for k, rng in init_params.items()}
    start_proc = apply_preprocessing(img, start_params)
    start_score = score_image(start_proc, template)

    counter = count()
    open_set = []
    heapq.heappush(open_set, (-heuristic(start_params, start_score), next(counter), start_params, start_proc, start_score))
    closed = set()

    best_score = start_score
    best_proc = start_proc
    best_params = start_params.copy()

    while open_set:
        _, _, params, proc, score = heapq.heappop(open_set)
        key = tuple(sorted(params.items()))
        if key in closed:
            continue
        closed.add(key)
        if score > best_score:
            best_score, best_proc, best_params = score, proc.copy(), params.copy()

        for k, rng in init_params.items():
            low, high = rng
            deltas = np.linspace(-(high-low)*0.1, (high-low)*0.1, 3)
            for delta in deltas:
                if abs(delta) < 1e-3:
                    continue
                p2 = params.copy()
                p2[k] = np.clip(p2[k] + delta, low, high)
                key2 = tuple(sorted(p2.items()))
                if key2 in closed:
                    continue
                proc2 = apply_preprocessing(img, p2)
                sc2 = score_image(proc2, template)
                heapq.heappush(open_set, (-heuristic(p2, sc2), next(counter), p2, proc2, sc2))
