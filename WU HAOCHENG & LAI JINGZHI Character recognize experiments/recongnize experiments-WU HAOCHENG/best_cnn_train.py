...
def train_model(model,num_epochs, train_loader, loss_fn, optimizer,number):
    ...
    for epoch in range(num_epochs):
        ...
        if max_acc<val_acc:
            torch.save(model, f"final_cnn_best_{number}.pth")
...
def evaluate_state(state, num_classes, train_loader, val_loader,model_num):
    ...
    train_model(model,300,train_loader,loss_fn,optimizer,model_num)
    result=save_eval_metrics(model, val_loader, number_model=model_num, class_names=full_dataset.classes)
    return result

# --------- 模型加载 ----------
layers=[0, 1, 1, 1, 1, 1, 0, 1]
kernel=[7, 5, 5, 3, 3, 3, 3, 3]
num_classes = 37  # 修改为你实际分类数量
# --------- 图片预处理 ----------
...
#训练模型
print(evaluate_state(layers+kernel, num_classes, train_loader, val_loader,100))